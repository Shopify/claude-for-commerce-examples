...
def create_merchant_portal(
    ...
    *,
    ...
    """One store's portal. The access token goes into the Admin client and nowhere else:
    the backend holds the client, and the agent holds the backend, so no layer above the
    transport can read it and no tool result can carry it.
