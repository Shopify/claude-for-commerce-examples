...
def build_merchant_router(
    *,
    ...
    async def chat(request: MerchantChatRequest, record: CurrentSession) -> StreamingResponse:
        ...
        return stream_turn(agent, sessions, record, context(record), env_hint=f"{example_dir}/.env")
    ...
    # ``:path`` because a listing id is whatever the store calls a listing, and a real
    # commerce platform's ids are not always URL path segments: a Shopify global id is
    # ``gid://shopify/Product/1``, and its slashes survive no amount of percent-encoding
    # (an ASGI server decodes the path before the router sees it). Every id that matched
    # the plain converter still matches this one.
    ...
    @router.post("/changes/{change_id}/discard")
